	...
	<div class="list_main">
      <table class="list_search">
      		<tr class="date">
      			...
      			<td class="date_btn"><input type="button" id="last_year" value="작년"></td>
  ...
  <script>
  	...
  	$(function() {
  		...
  		$("#search").click(function() {
  			...
  			var cal1 = $("#cal1").val();
  			var cal2 = $("#cal2").val();
  			$.getJSON("goodslist_search", {"userid":userid, "cal1":cal1, "cal2":cal2}, function(data) {
  				if(data['count'] != 0) {
  					var pageNum = data['PageNum'];
  	  				...
  	  				$("#page").on("click", "#prev", function() {
  	   				...
  	   				$("#page").on("click", "#first" , function() {
  	  					$("#goodslist").empty();
  			  			$("#goodslist").append("<th>순번</th><th>헌혈의집</th><th>당첨일자</th><th>경품</th>");
  			  			var page = $(this).text();
  			  			...
  			  			for(i - 1; i < j; i++){
  							$("#goodslist").append("<tr><td>" + (i + 1) + "</td><td>" + data['list'][i].bhname + "</td><td>" + data['list'][i].goodsdate + "</td><td>" + data['list'][i].goods + "</td></tr>");
  						}
  	  				});
  	  		  		$("#first").trigger("click");
  				}else {
